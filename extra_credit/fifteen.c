...
//  int d;
 ...
 {
    int d; //declaring int variable d
     ...
     int **board = malloc(sizeof(int*)*d); //initializing the board double pointer, and assigning memory

    for(int i = 0; i < d; i++){
        board[i] = malloc(sizeof (int) * d); //assigning values to each index
    }
