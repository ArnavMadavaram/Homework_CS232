...
int contains(const struct listNode *pNode, const char *addr)
{
  if (pNode == NULL)
  {
    return 0; // returns 0, when the address is not found
  }
  if (strcmp(pNode->addr, addr) == 0)
  {
    return 1; // returns 1, when the address is found
  }
  return contains(pNode->next, addr); // Recursive call
}
...
void insertBack(struct listNode *pNode, const char *addr)
{
  if (pNode->next == NULL)
  {
    ...
    if (pNode->next == NULL) // creating a new node if we reach the end
    {
  ...
  {
    insertBack(pNode->next, addr); // recursive call
  }
}
...
void printAddresses(const struct listNode *pNode)
{
  ...
  printf("%s\n", pNode->addr); // printing the current address
...
void destroyList(struct listNode *pNode)
{
  if (pNode == NULL)
  {
    return; // returns if there are no more nodes to be freed
  }
